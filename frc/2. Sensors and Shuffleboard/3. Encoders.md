...
We frequently use these encoders to measure the revolutions performed by a Talon SRX motor attached to a motor such as a CIM motor. When electronics uses these encoders, they plug directly into the Talon SRX, so we can access it through there. We will start with the code below.
...
One issue that can arise when we use the aforementioned system is that our encoder values can start to drift and not match the real world. For instance, with mechanical issues such as gearbox slop, chain slipping, etc., our encoder value can start to not match the desired value. This is generally why we prefer to use limit switches instead of encoders for limiting our mechanisms. However if it is unfeasible to place limit switches on our mechanism, we may have to use encoders. However if the encoder values start to drift, we may find that our software stops prevent us from reaching our full range of motion. In this case, we must implement overrides to ensure that if this case should happen, the operator can override the soft stop and allow the mechanism to continue moving.
