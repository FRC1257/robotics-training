...
```
First, the solenoids are declared. Since this subsystem uses pneumatics(air) instead of motors, solenoids replace motor controllers in the code. The solenoids are responsible for determining if air goes to the pneumatic cylinders or not. Since the claw we are making has a pneumatic cylinder on each side, there are two solenoids declared. Each of the solenoids are a part of the class `DoubleSolenoid` which means that the solenoids are double acting. 
...
## Constants
Below is the constants file which is self-explanatory and is gone over in the roller intake subsystem tutorial. Therefore the are no comments explaining how it works.
```java
package frc.robot;
